...
async def _handle_callback(callback: dict):
    ...
    if data.startswith("pause:"):
        ...
        # Beta survey decommissioned (2026-07-14). Old messages in chats
        # may still carry the buttons — ack with a polite "closed" toast.
        try:
            await bot.answer_callback_query(
                callback_query_id=callback_id,
                text="Ce sondage est terminé — merci pour ton aide pendant la beta ! 🙏",
            )
        except Exception:
            pass
    ...
    try:
        ...
        await bot.send_message(
            ...
            text=(
                ...
                "Petit conseil pour la prochaine fois : ouvre le lien du deal "
                "avant de donner ton avis 👍/👎. Tu vois la dispo réelle, les "
                "dates exactes et le prix final — ton feedback nous aide "
                "vraiment plus quand il s'appuie sur l'offre réelle. 🚀"
            ),
        )
    except Exception as e:
